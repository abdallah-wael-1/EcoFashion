...
import { User, Mail, Shield, Check, Lock, Eye, EyeOff, AlertTriangle, X } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import ProfileImage from '../../components/common/ProfileImage';
import { useRoles } from '../../hooks/useRoles';
import { roleMetadata } from '../../config/rolesConfig';
...
const UserProfile = () => {
  ...
  // Delete confirmation modal — regular users only
  ...
  const onSave = () => {
    ...
    if (hasChanges) {
      ...
      showToast('success', 'Done editing.');
    }
  ...
  /* ══════════════════════════════════════════════════════════════
     ADMIN VIEW — minimal: avatar + name + email + password only
  ══════════════════════════════════════════════════════════════ */
  if (isAdmin(user)) {
    return (
        ...
        <div className="mx-auto max-w-3xl space-y-6">
          ...
          <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
            <div className="relative h-36 overflow-hidden">
              <div className="w-full h-full bg-gradient-to-br from-gray-200 via-gray-100 to-gray-200 dark:from-gray-800 dark:via-gray-800 dark:to-gray-900">
                <div className="absolute inset-0 opacity-25"
                  style={{ backgroundImage: 'linear-gradient(120deg, rgba(255,255,255,0.35) 0%, rgba(255,255,255,0) 40%, rgba(255,255,255,0.35) 100%)' }} />
              </div>
            </div>
            ...
            <div className="px-8 pb-8">
              <div className="flex items-end justify-between -mt-14 mb-5">
                <ProfileImage
                  ...
                  onEditComplete={() => showToast('success', 'Profile image updated successfully!')}
                />
                {!isEditing ? (
              ...
              <div className="mb-4">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{user?.name || 'Admin'}</h1>
  ...
  /* ══════════════════════════════════════════════════════════════
     REGULAR USER VIEW — full profile
  ══════════════════════════════════════════════════════════════ */
        ...
        <div className="rounded-2xl border border-gray-200/80 dark:border-gray-800 bg-white/90 dark:bg-gray-900/90 backdrop-blur-sm overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300">
          ...
          <div className="px-8 pb-8">
            ...
            <div className="mt-6 grid grid-cols-3 gap-4">
              {[
                { icon: Shield, label: 'EcoCredits', value: ecoCredits ?? 0,      color: 'text-green-600 dark:text-green-400' },
                { icon: Shield, label: 'Trust Score', value: `${trustScore ?? 0}`, color: 'text-amber-500 dark:text-amber-400' },
                { icon: Shield, label: 'Role',        value: roleMetadata[activeRole]?.name || activeRole || 'Buyer', color: 'text-blue-600 dark:text-blue-400' },
              ].map(s => (
